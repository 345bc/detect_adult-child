...
@st.cache_resource
def load_model():
    # Tai mo hinh YOLO custom tu file trong so cua nhom
    return YOLO("best.pt")

model = load_model()

# Khoi tao cac bien luu tru trang thai va lich su thong ke
if "adult_in" not in st.session_state: st.session_state.adult_in = 0
if "child_in" not in st.session_state: st.session_state.child_in = 0
if "adult_out" not in st.session_state: st.session_state.adult_out = 0
if "child_out" not in st.session_state: st.session_state.child_out = 0
if "counted_in_ids" not in st.session_state: st.session_state.counted_in_ids = set()
if "counted_out_ids" not in st.session_state: st.session_state.counted_out_ids = set()
if "track_history" not in st.session_state: st.session_state.track_history = {}
if "log_data" not in st.session_state: 
    st.session_state.log_data = pd.DataFrame(columns=["Thoi_Gian", "Doi_Tuong", "Chieu_Di", "Chieu_Cao_CM"])
...
conf_threshold = st.sidebar.slider(
    "Ngưỡng tin cậy (Confidence):", 0.1, 1.0, 0.4, 0.05,
...
line_position = st.sidebar.slider(
    "Vị trí vạch kiểm soát (Tỷ lệ %):", 10, 90, 50, 5,
...
bias = st.sidebar.number_input(
    "Sai số hệ thống (bias):", value=45.0,
...
def update_ui_metrics():
    ...
    slot_revenue.metric(label="Ước Tính Doanh Thu Vé Vào Cổng (VNĐ)", value=f"{revenue:,}")
            ...
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for box, id_obj, cls in zip(boxes, ids, clss):
                    ...
                    if len(st.session_state.track_history[id_obj]) > 1:
                        for i in range(1, min(20, len(st.session_state.track_history[id_obj]))):
                    ...
                    if len(st.session_state.track_history[id_obj]) > 1:
                        ...
                        current_cx, current_cy = st.session_state.track_history[id_obj][-1]
                        
                        is_in = False
                        is_out = False
                        
                        if direction == "Từ Trên xuống Dưới":
                            if prev_cy < cy_line <= current_cy: is_in = True
                            elif prev_cy > cy_line >= current_cy: is_out = True
                        elif direction == "Từ Dưới lên Trên":
                            if prev_cy > cy_line >= current_cy: is_in = True
                            elif prev_cy < cy_line <= current_cy: is_out = True
                        elif direction == "Từ Trái sang Phải":
                            if prev_cx < cx_line <= current_cx: is_in = True
                            elif prev_cx > cx_line >= current_cx: is_out = True
                            ...
                            if is_in and id_obj not in st.session_state.counted_in_ids:
                                if cls == 0: st.session_state.adult_in += 1
                                elif cls == 1: st.session_state.child_in += 1
                                st.session_state.counted_in_ids.add(id_obj)
                                
                                new_row = pd.DataFrame([{"Thoi_Gian": now_str, "Doi_Tuong": obj_type, "Chieu_Di": "VÀO", "Chieu_Cao_CM": round(estimated_height, 1)}])
                            ...
                            if is_out and id_obj not in st.session_state.counted_out_ids:
                                if cls == 0: st.session_state.adult_out += 1
                                elif cls == 1: st.session_state.child_out += 1
                                st.session_state.counted_out_ids.add(id_obj)
                                
                                new_row = pd.DataFrame([{"Thoi_Gian": now_str, "Doi_Tuong": obj_type, "Chieu_Di": "RA", "Chieu_Cao_CM": round(estimated_height, 1)}])
                                st.session_state.log_data = pd.concat([st.session_state.log_data, new_row], ignore_index=True)
                                update_ui_metrics()
                ...
                if not st.session_state.log_data.empty():
                    chart_data = st.session_state.log_data.groupby(["Thoi_Gian", "Chieu_Di"]).size().unstack(fill_value=0)
