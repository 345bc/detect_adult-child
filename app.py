...
if "log_list" not in st.session_state: st.session_state.log_list = []
...
if source_type == "Hình ảnh":
    uploaded_file = main_col1.file_uploader("Tải ảnh lên...", type=["jpg", "jpeg", "png"])
    ...
    if source_type == "Video (File)":
        uploaded_video = main_col1.file_uploader("Tải video lên...", type=["mp4", "avi", "mov"])
    ...
    if run_tracking and video_path is not None:
        ...
        while cap.isOpened():
            ...
            if direction in ("Từ Trên xuống Dưới", "Từ Dưới lên Trên"):
                ...
                cv2.putText(frame, f"Line Y: {cy_line}", (10, cy_line - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            else:
                cx_line = int(w * (line_position / 100))
                cv2.line(frame, (cx_line, 0), (cx_line, h), (255, 0, 0), 3)
                cv2.putText(frame, f"Line X: {cx_line}", (cx_line + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            ...
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for box, id_obj, cls in zip(boxes, ids, clss):
                    ...
                    coord_text = f"Y:{cy}" if direction in ("Từ Trên xuống Dưới", "Từ Dưới lên Trên") else f"X:{cx}"
            ...
            if st.session_state.log_list:
                ...
                chart_data = df_chart.groupby(["Thoi_Gian", "Chieu_Di"]).size().unstack(fill_value=0)
